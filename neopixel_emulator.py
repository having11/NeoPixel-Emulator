...
class NeoPixel_Emulator(pyglet.window.Window):
    def __init__(self):
        super(NeoPixel_Emulator,self).__init__(width=1765, height=400)
    ...
    def draw_LEDs(self, led_number):
        for led in range(led_number):
            ...
            self.color_sprites.append(pyglet.sprite.Sprite(img=pyglet.image.load('circle.png'),batch=self.batch,x=(led-50*(led//50))*35,y=self.height-34-((led//50)*34),group=self.color_group))
